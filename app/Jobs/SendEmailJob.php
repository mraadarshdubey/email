...
class SendEmailJob implements ShouldQueue
{
    ...
    /**
     * Throttle sends to stay under the provider's rate limit (Amazon SES).
     * When the limit is hit the job is released back to the queue with a short
     * delay instead of failing — retryUntil() bounds the overall retry window.
     */
    ...
    public function handle(): void
    {
        try {
            ...
            // Never mail an address that opted out or hard-bounced. This is
            // checked at send time (not just at dispatch) so opt-outs during a
            // long campaign take effect immediately.
            $suppressed = EmailContact::where('email', $this->email)
                ->whereIn('status', ['unsubscribed', 'bounced'])
                ->exists();

            if ($suppressed) {
                Log::info('Suppressed address skipped', ['email' => $this->email]);
                return;
            }
            ...
            // Send the email (recipient is passed so the mailable can build the
            // per-recipient one-click unsubscribe link and headers)
