...
class ContactController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $automation = app(AutomationRuleService::class);
        $sequences = app(SequenceEnrollmentService::class);
        $automation->fireContactCreated($contact);

        if ($request->filled('tags')) {
            $contact->tags()->sync($request->tags);
            foreach ($request->tags as $tagId) {
                $automation->fireTagAdded($contact, (int) $tagId);
                $sequences->enrollForTag($contact, (int) $tagId);
            }
        }
    ...
    public function update(Request $request, EmailContact $contact)
    {
        ...
        $syncResult = $contact->tags()->sync($request->tags ?? []);

        $automation = app(AutomationRuleService::class);
        $sequences = app(SequenceEnrollmentService::class);
        foreach ($syncResult['attached'] as $tagId) {
            $automation->fireTagAdded($contact, (int) $tagId);
            $sequences->enrollForTag($contact, (int) $tagId);
        }
        ...
        switch ($request->action) {
            ...
            case 'add_tag':
                ...
                foreach ($contacts as $contact) {
                    $result = $contact->tags()->syncWithoutDetaching([$request->tag_id]);
                    if (!empty($result['attached'])) {
                        $automation->fireTagAdded($contact, (int) $request->tag_id);
                        $sequences->enrollForTag($contact, (int) $request->tag_id);
                    }
                }
